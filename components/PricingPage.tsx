  ...
  const getCardStyles = (tier: string, popular: boolean) => {
    ...
    switch (tier) {
      case "basic":
        return `${baseStyles} ${
          popular
            ? "border-green-500 dark:border-green-400 border-2 shadow-xl dark:shadow-2xl lg:scale-105 bg-gradient-to-br from-green-600 via-emerald-700 to-green-800 dark:bg-gradient-to-br dark:from-green-700 dark:via-emerald-800 dark:to-green-900"
            : "border-green-200 dark:border-green-700 hover:shadow-lg bg-gradient-to-br from-green-600 via-emerald-700 to-green-800 dark:bg-gradient-to-br dark:from-green-700 dark:via-emerald-800 dark:to-green-900 hover:border-green-300 dark:hover:border-green-600"
        }`
      case "standard":
        return `${baseStyles} ${
          popular
      ...
      case "premium":
        return `${baseStyles} ${
          popular
            ...
            : "border-purple-200 dark:border-purple-700 hover:shadow-lg bg-gradient-to-br from-purple-600 via-pink-700 to-purple-800 dark:bg-gradient-to-br dark:from-purple-700 dark:via-pink-800 dark:to-purple-900 hover:border-purple-300 dark:hover:border-purple-600"
        }`
      default:
        return baseStyles
    }
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16 relative z-10">
            ...
            <div className="text-center mb-12 lg:mb-16">
              ...
              <p className="text-lg sm:text-xl text-gray-300 dark:text-gray-400 max-w-3xl mx-auto mb-6 sm:mb-8 px-4 font-['Inter',_'system-ui',_sans-serif]">
                Choose the perfect website package to transform your business online. Each solution is crafted to
                deliver measurable results and drive real growth for your Uganda-based business.
              </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 max-w-7xl mx-auto mb-12 lg:mb-16">
              {packages.map((pkg, index) => (
                <Card
                  key={pkg.name}
                  className={`${getCardStyles(pkg.tier, pkg.popular)} ${index === 1 ? "lg:mt-0" : "lg:mt-4"} backdrop-blur-sm`}
                >
                  {pkg.popular && (
                    <Badge
                  ...
                  <CardHeader className="text-center pb-4 px-4 sm:px-6">
                    <CardTitle className="text-xl sm:text-2xl font-bold text-white dark:text-white font-['Inter',_'system-ui',_sans-serif]">
                      {pkg.name}
                    </CardTitle>
                    <CardDescription className="text-gray-100 dark:text-gray-100 mt-2 text-sm sm:text-base font-['Inter',_'system-ui',_sans-serif]">
                      {pkg.description}
                    </CardDescription>
                    <div className="mt-4 sm:mt-6">
                      <span className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white dark:text-white">
                        {formatPrice(pkg.priceUGX, pkg.priceUSD)}
                      </span>
                      <span className="text-gray-200 dark:text-gray-200 ml-2 text-sm">one-time</span>
                    </div>
                  </CardHeader>
                    ...
                    <ul className="space-y-2 sm:space-y-3">
                      {pkg.benefits.map((benefit, benefitIndex) => (
                        <li key={benefitIndex} className="flex items-start gap-2 sm:gap-3">
                          <Check className="w-4 h-4 sm:w-5 sm:h-5 text-green-300 dark:text-green-300 mt-0.5 flex-shrink-0" />
                          <span className="text-gray-100 dark:text-gray-100 text-xs sm:text-sm leading-relaxed font-['Inter',_'system-ui',_sans-serif]">
                            {benefit}
                          </span>
                        </li>
                      ))}
            ...
            <div className="text-center bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-3xl p-6 sm:p-8 shadow-2xl max-w-4xl mx-auto border border-purple-200/30 dark:border-purple-500/30">
              ...
              <p className="text-gray-600 dark:text-gray-300 mb-6 text-base sm:text-lg px-4 font-['Inter',_'system-ui',_sans-serif]">
                Join hundreds of successful Uganda businesses who have already made the smart investment in their
                digital future. Let's discuss which package is perfect for your goals.
              </p>

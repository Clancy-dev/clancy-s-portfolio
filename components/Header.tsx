              ...
              <div className="p-3 xs:p-4 sm:p-6 space-y-2 xs:space-y-3 flex-1 overflow-y-auto">
                {links.map((link, index) => (
                  <Link
                    ...
                    onClick={() => setIsMenuOpen(false)}
                    style={{ animationDelay: `${index * 50}ms`, animation: isMenuOpen ? "slideInUp 0.3s ease-out forwards" : "none" }}
                  >
                    <div
                      className={cn(
                        "flex-shrink-0 p-1.5 xs:p-2 rounded-lg xs:rounded-xl transition-all duration-300",
                    ...
                    <span className="font-['Inter',_'system-ui',_sans-serif] font-medium flex-1 text-left">{link.name}</span>
                  </Link>
                ))}
